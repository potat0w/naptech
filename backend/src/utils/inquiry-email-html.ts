...
function enquiryCard(data: InquiryEmailData) {
  ...
  return `<table width="100%" cellpadding="0" cellspacing="0" role="presentation" style="margin-top:20px;border:1px solid #ede9f3;border-radius:12px;background-color:#faf8f4;overflow:hidden;">
    <tr>
      <td style="padding:16px 18px;background-color:#3f2d62;">
        <p style="margin:0 0 4px;font-size:11px;font-weight:bold;text-transform:uppercase;letter-spacing:0.06em;color:rgba(255,255,255,0.75);">Subject</p>
        <p style="margin:0;font-family:Georgia,'Times New Roman',serif;font-size:20px;color:#ffffff;">${escapeHtml(data.subject)}</p>
      </td>
    </tr>
    <tr>
...
export function buildInquiryNotificationEmailHtml(
  data: InquiryEmailData,
  adminUrl: string
) {
  const body = `
    <p style="margin:0 0 8px;font-size:16px;line-height:1.6;color:#404040;">
      A new enquiry was submitted on the Naptec website.
    </p>
    <p style="margin:0;font-size:15px;line-height:1.6;color:#6b6560;">
      Reply to the contact details below or review it in the admin portal.
    </p>
    ${enquiryCard(data)}
    ${ctaButton("View in admin portal", adminUrl)}`;

  return wrapEmailHtml({
    title: `New enquiry: ${data.subject}`,
    preheader: `New enquiry from ${data.fullName}: ${data.subject}`,
    headerBg: "#3f2d62",
    headline: "New website enquiry",
    body,
  });
}
