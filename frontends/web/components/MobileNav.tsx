...
import { dashboardPathForRole, isExternalUrl, roleLabel } from "@/lib/portal/role";
...
function footerLinksForRole(role: "admin" | "caregiver" | "client" | undefined) {
  ...
  if (role === "admin") {
    const href = dashboardPathForRole("admin");
    links.push({ label: "Admin portal", href, external: isExternalUrl(href) });
  } else if (role === "caregiver") {
    const href = dashboardPathForRole("caregiver");
    links.push({ label: "Caregiver portal", href, external: isExternalUrl(href) });
  } else if (role === "client") {
    links.push({ label: "Book care", href: "/book" });
  }
  return links;
}
