...
const caregiverNav: NavItem[] = [
  ...
  { href: "/caregiver/tasks", label: "Assigned Tasks", icon: ClipboardList },
...
const adminNav: NavItem[] = [
  ...
  { href: "/admin/bookings", label: "Booking requests", icon: Inbox },
...
export default function PortalShell({ portal, title, children }: PortalShellProps) {
  const { user, logout } = useAuth();
  const pathname = usePathname();
  const router = useRouter();
  const [mobileOpen, setMobileOpen] = useState(false);
  const nav = portal === "admin" ? adminNav : caregiverNav;
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-xs font-medium uppercase tracking-wide text-brand">
                {roleLabel(portal === "admin" ? "admin" : "caregiver")} portal
              </p>
